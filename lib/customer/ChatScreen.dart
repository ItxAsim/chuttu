...
class ChatScreen extends StatefulWidget {
  ...
  ChatScreen({required this.userId, required this.professionalId});
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    _firestore.collection('messages').snapshots().listen((snapshot) {
      for (var message in snapshot.docs) {
        if (message['receiverId'] == widget.userId && !message['seen']) {
  ...
  Future<void> _sendMessage(String message) async {
    // Send text messages as usual
    _firestore.collection('messages').add({
      'text': message,
      'senderId': widget.userId,
      'receiverId': widget.professionalId,
  ...
  Widget _buildMessageItem(DocumentSnapshot message) {
    ...
    final bool isUserMessage = senderId == widget.userId; // Check if the message is sent by the user
